...
const uploadThesis = async (req, res) => {
  ...
  try {
    ...
    const uploadPath = `./uploads/teze/${fileName}`;

    // Mută fișierul în folderul de upload
    await file.mv(uploadPath);

    // Actualizează înregistrarea în baza de date
    const [result] = await conn.execute(
      `UPDATE lucrare 
       SET fisier = ?, stare = 'In evaluare'
       WHERE id_student = ? 
    ...
    if (result.affectedRows === 0) {
      return res.status(404).json(
        createResponse(false, "No thesis found for this student")
      );
    }

    return res.status(200).json(
      createResponse(true, "Thesis uploaded successfully", { fileName })
    );
  } catch (err) {
    return res.status(500).json(
